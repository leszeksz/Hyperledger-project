...
function createOrg4 {
  ...
  set -x
	fabric-ca-client enroll -u https://peer0:peer0pw@localhost:11064 --caname ca-org4 -M "${PWD}/../organizations/peerOrganizations/org4.example.com/peers/peer0.org4.example.com/msp" --csr.hosts peer0.org4.example.com --tls.certfiles "${PWD}/fabric-ca/org4/tls-cert.pem"
  { set +x; } 2>/dev/null
